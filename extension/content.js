...
const escutador = (url) => {
    if (url == "www.facebook.com") {
        ...
        if (
            ...
            window.location.href.split("/")[4] == "t"
        ) {
            // console.log(123)
            ...
            let chatI = document.querySelectorAll(
            ...
            chatI.forEach((element) =>
                element.addEventListener("keydown", contaEnterComenInsta)
            );

            cliqueI = document.querySelectorAll(
            ...
            cliqueI.forEach((element) => {
                element.addEventListener("click", contaCliqueComenInsta);
            });
        }
    }
};
...
function contaCliqueComenInsta(event) {
    if (event.srcElement.textContent == "Publicar") {
        salvarMsgs("comentarioInstagram");
    }
}
function contaEnterComenInsta(event) {
    // console.log(event.srcElement.textContent);
    if (event.which === 13 && event.srcElement.textContent != "") {
        setTimeout(() => {
            // console.log(event.srcElement.textContent);
    ...
    ultimaC = event.srcElement.textContent;
}
